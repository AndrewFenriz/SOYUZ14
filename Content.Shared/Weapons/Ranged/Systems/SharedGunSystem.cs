...
public abstract partial class SharedGunSystem : EntitySystem
{
    ...
    private void OnStopShootRequest(RequestStopShootEvent ev, EntitySessionEventArgs args)
    {
        ...
        if (user == null ||
            ...
            !TryGetGun(user.Value, out _, out var userGun))
        {
            return;
        }
    ...
    public void ResetMechGun(EntityUid gunUid, GunComponent gun)
    {
    ...
    private void HandleMechShooting(EntityUid user, EntityUid gunUid, GunComponent gun)
    {
        if (!TryComp<MechPilotComponent>(user, out var mechPilot) ||
            !TryComp<MechComponent>(mechPilot.Mech, out var mech))
            return;

        ResetMechGun(gunUid, gun);
...
private void AttemptShoot(EntityUid user, EntityUid gunUid, GunComponent gun)
{
    ...
    // Обработка мехов
    if (TryComp<MechPilotComponent>(user, out _))
    {
        HandleMechShooting(user, gunUid, gun);
        return;
    }
